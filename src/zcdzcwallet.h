...
#ifndef HCASH_ZHCASHWALLET_H
#define HCASH_ZHCASHWALLET_H
...
public:
    CzHCASHWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZHCASH(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZHCASH(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //HCASH_ZHCASHWALLET_H
