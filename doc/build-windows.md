...
example /usr/src/HCASH, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/HCASH/HCASH.git
    sudo chmod -R a+rw HCASH
...
way. This will install to `c:\workspace\HCASH`, for example:

    make install DESTDIR=/mnt/c/workspace/HCASH
...
causes two of the HCASH executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the HCASH code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the HCASH source code).
